...
실제로 쓰이는지 한눈에 보이는 곳이 없었다. 만든 사람의 환경을 2026-08-22에 재 보니 MCP 도구 정의 61k, 메모리·규칙 34.5k, 스킬 목록 10k가
매 세션 실리고 있었고 그 가운데 30일간 한 번도 부르지 않은 것이 태반이었다.
...
처리한 뒤 `data/actions.jsonl`에 적는다. 삭제는 하지 않고 규칙·스킬 원본 디렉터리는 건드리지 않는다.
