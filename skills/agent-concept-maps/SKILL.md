...
description: 코딩 에이전트를 처음 쓰는 사람에게 다섯 장의 개념 지도를 순서대로 펼쳐 보이며 설명하고, 같은 꼴의 장을 더 만들 때 쓴다. 「에이전트가 뭔지 설명해 줘」「개념 지도 보여 줘」「이 지도에 한 장 더」가 방아쇠다.
---

# 에이전트 개념 지도

저장소 루트 `docs/agent-concept-maps/` 아래 다섯 장이 교재다. 자기완결 HTML(인라인 SVG, 외부 의존은 Google Fonts뿐)이라 브라우저로 그냥 열린다.
순서가 있다 — 뒤 장이 앞 장의 말을 쓴다.
...
다섯 장을 한 번에 훑을 때는 한 장에 십 분, 장 사이에 「지금까지 이해한 것을 자기 말로」 한 번씩.

## 만들기 — 한 장 더 얹기

같은 꼴의 장은 같은 저장소의 `skills/explain-with-pictures` 스킬로 만든다. 그 스킬이 「그림 한 장이
있는 자기완결 HTML」을 내므로, 여기 장들과 같은 규약이다.

1. 답할 질문 하나를 제목으로 정한다(위 표처럼 의문문). 한 장에 질문 하나.
2. explain-with-pictures에 그 질문과, 앞 장에서 이미 쓴 낱말 목록을 준다 — 같은 개념을 다른 이름으로 부르지 않게.
3. 나온 HTML을 `docs/agent-concept-maps/06-<주제>.html`로 두고 `docs/agent-concept-maps/index.html` 차례에 한 줄 더한다. 앞 장들의 `nav.back` 링크 형식을 따른다.
...
- 장 하나만 가져갈 때는 그 HTML 파일 하나면 된다 — 자기완결이라 다른 파일을 끌고 가지 않는다.
